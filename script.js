(function(){
	...
	document.addEventListener('DOMContentLoaded', function(){
		...
		// Theme initialization and toggle wiring (True Black <-> Light)
		(function(){
			...
			function applyTheme(t){
				...
				if(themeToggleBtn){
					...
					if(t === 'dark') themeToggleBtn.classList.add('active'); else themeToggleBtn.classList.remove('active');
				}
			}
			const stored = (function(){ try{ return localStorage.getItem('fp-theme'); }catch(e){return null;} })();
			if(stored) applyTheme(stored);
			if(themeToggleBtn){
				// sync initial aria-pressed
				themeToggleBtn.setAttribute('aria-pressed', document.documentElement.classList.contains('dark') ? 'true' : 'false');
		...
		let waterCount = (function(){ try{ return parseInt(localStorage.getItem('fp-water') || '0',10) || 0; }catch(e){return 0;} })();
		...
		if(qs('waterReset')) qs('waterReset').addEventListener('click', ()=>{ waterCount = 0; try{ localStorage.setItem('fp-water','0'); }catch(e){} renderWater(); });
		renderWater();
		...
		// Load saved values if any (leave fields empty for new users)
		loadLast();
		// ensure inputs have no hardcoded defaults: if element has value attribute in HTML, clear it for new users
		['weight','height','age'].forEach(id=>{ const el=qs(id); if(el && !localStorage.getItem('fp-last')) el.value = ''; });
			...
			if(key === 'ppl'){
				...
				workoutDetails.innerHTML = `
					<div class="text-slate-200">
						...
						<div class=\"mt-2 workout-table-wrapper text-sm\"> 
						<table class=\"workout-table w-full table-auto text-sm\"><thead><tr class=\"text-amber-300\"><th class=\"px-2 py-1\">Day</th><th class=\"px-2 py-1\">Focus</th><th class=\"px-2 py-1\">Example Exercises</th></tr></thead><tbody class=\"text-slate-200\">\
		...
		// Mobile nav wiring: toggle mobile panes (Home/Meals/Workout)
		(function(){
			...
			function showView(id){
				...
				const btn = buttons.find(b=>b.getAttribute('data-view')===id);
		...
		if(reset){
			reset.addEventListener('click', function(){
				clearResults();
				// clear stored last inputs and water
				try{ localStorage.removeItem('fp-last'); localStorage.removeItem('fp-water'); }catch(e){}
				// clear form fields
				['weight','height','age'].forEach(id=>{ const el=qs(id); if(el) el.value=''; });
				// reset water UI
